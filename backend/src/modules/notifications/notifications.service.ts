...
import { PlatformNotification, NotificationDocument, NotificationType } from './schemas/notification.schema';
import { CreateNotificationDto } from './dto/create-notification.dto';
import { Store, StoreDocument, StoreStatus } from '../stores/schemas/store.schema';
...
export class NotificationsService {
  ...
  constructor(
    @InjectModel(PlatformNotification.name) private notificationModel: Model<NotificationDocument>,
  ...
  async create(createDto: CreateNotificationDto): Promise<PlatformNotification> {
    const notification = new this.notificationModel({
      ...createDto,
      storeId: createDto.storeId ? new Types.ObjectId(createDto.storeId) : undefined,
    });
  ...
  async checkExpiringSubscriptions() {
    ...
    const storesExpiringSoon = await this.storeModel.find({
    ...
    const expiredStores = await this.storeModel.find({
      status: StoreStatus.ACTIVE,
      'subscription.renewalDate': {
        $lt: today,
      },
