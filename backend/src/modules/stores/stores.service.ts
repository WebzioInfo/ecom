...
export class StoresService {
  ...
  async create(createStoreDto: CreateStoreDto): Promise<Store> {
    const existing = await this.storeModel.findOne({ slug: createStoreDto.slug });
    if (existing) {
  ...
  async findOne(id: string): Promise<Store> {
    const store = await this.storeModel.findById(id).populate('ownerId', 'name email roles').exec();
    if (!store) {
  ...
  async findBySlug(slug: string): Promise<Store> {
    const store = await this.storeModel.findOne({ slug }).populate('ownerId', 'name email roles').exec();
    if (!store) {
